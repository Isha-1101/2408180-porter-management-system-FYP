...
export const respondToTeamInvitation = async (req, res) => {
  ...
  try {
    ...
    const joinRequest = await TeamJoinRequest.findById(requestId).session(session);
    if (!joinRequest) {
      await session.abortTransaction();
      session.endSession();
      return res.status(404).json({
    ...
    if (action === "ACCEPTED") {
      ...
      const team = await PorterTeam.findById(joinRequest.teamId).session(session);
      ...
      if (porter && team) {
        ...
        if (teamOwner) {
          const teamOwnerUser = await User.findById(teamOwner.userId).session(session);
          if (teamOwnerUser) {
