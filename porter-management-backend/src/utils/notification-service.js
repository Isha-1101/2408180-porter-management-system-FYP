...
export const notifyPorter = async (porterId, booking, distance) => {
  ...
  // Emit live SSE to porter
  sseService.sendToPorter(porter._id, "new-booking-request", notification);
...
export const notifyUser = async (
  ...
  const messages = {
    ...
    BOOKING_REJECTED:
      "Your booking was rejected. Searching for another porter...",
  ...
  const notification = await createNotification({
    ...
    message:
      customMessage || messages[notificationType] || "Booking status updated",
    data: {
      bookingId: booking._id,
  ...
  // Map notification types to SSE events expected by frontend
  if (
    notificationType === "BOOKING_ACCEPTED" ||
    notificationType === "BOOKING_CONFIRMED"
  ) {
    sseService.sendToUser(userId, "booking-confirmed", notification);
  } else if (notificationType === "BOOKING_CANCELLED") {
    sseService.sendToUser(userId, "booking-cancelled", notification);
  } else if (notificationType === "BOOKING_COMPLETED") {
    sseService.sendToUser(userId, "booking-completed", notification);
  }
